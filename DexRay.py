...
def parseargs():
    ...
    parser.add_argument("-p", "--path",
                        help="The path to the directory that contains malware and goodware image folders", type=str,
                        required=True)
    parser.add_argument("-d", "--dir", help="The name of the directory where to save the model", type=str,
                        required=True)
    parser.add_argument("-f", "--file", help="The name of the file where to save the results of the evaluation",
...
def get_image(path_img):
    f_path = ""
    if isTest is True:
        f_path = "D:/DexRay/images/apk/apps_for_model/test/" + path_img.decode('ascii')
    else:
        f_path = "D:/DexRay/images/apk/apps_for_model/train/" + path_img.decode('ascii')
    # print(f_path)
...
def main(path_images, dir_name, file_name, CHANNELS, EPOCHS, BATCH_SIZE, IMG_SIZE, PATH_FILES, CLASS_NAMES):
    ...
    # create list of apk names for TEST
    path_to_test_images = "images/apk/apps_for_model/test"
    test_images_as_list = os.listdir(path_to_test_images)
    # get the slices of an array in the form of objects
    test_dataset = tf.data.Dataset.from_tensor_slices(test_images_as_list)
    ...
    #    print(i)
    # return 0
    length_test = len(test_images_as_list)
    ...
    # create list of apk names for TRAIN
    path_to_train_images = "images/apk/apps_for_model/train"
    train_images_as_list = os.listdir(path_to_train_images)
    # get the slices of an array in the form of objects
    train_dataset = tf.data.Dataset.from_tensor_slices(train_images_as_list)
    # apply the function to every element of the data set
    ...
    # create list of apk names for VALID -opt
    # path_to_valid_images = os.path.join(PATH_FILES, "model_train", "validation")
    # valid_images_as_list = os.listdir(path_to_valid_images)

    recall_list, precision_list, accuracy_list, f1_list = [], [], [], []
    # print("Skipping model creation! we have one !")
    ...
    cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath=os.path.join(dir_name, 'cp'),
    ...
    model.fit(train_dataset, shuffle=False, epochs=EPOCHS,
              callbacks=[es_callback, cp_callback], verbose=2)

    path_save_model = os.path.join(dir_name, 'model3')
    # model.save(path_save_model)
    print("Saving the model")
    tf.keras.models.save_model(model=model, filepath=path_save_model)
